...
var browsurr = require('./browser')
var deb = require('./debugger')
  ...
  var argv = yargs
    ...
    .options({
      ...
      'script': {
        alias: 's',
        requiresArg: true,
        desc: `Runs a given script open to debugging with PUPPETEER_WS_ENDPOINT environment variable.
               Executes command 'node --inspect=localhost:<port> <script> -- PUPPETEER_WS_ENDPOINT=<endpoint>'.

               Is it expected that the provided script (or any file required by it) runs the following:
               - puppeteer.connect(browserWSEndpoint: program.env.PUPPETEER_WS_ENDPOINT)
